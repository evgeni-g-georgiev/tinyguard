...
Step 1 is required; raw MIMII data must be on disk. The split step runs
automatically: `simulation/data/splits/<snr>/` is created from
`data/mimii_<snr>/` on the first run for that SNR.
...
simulation/outputs/runs/2026-04-25_00-14-23/
  ...
  plots/
    grid.png        score timeline grid, 4 cols (fused if len(channels) > 1)
...
simulation/
  ...
  node/
    ...
    group.py            Group: N nodes of one machine + sigma-weighted fusion
