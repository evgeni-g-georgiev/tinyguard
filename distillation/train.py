#!/usr/bin/env python3
"""
train.py — Distil YAMNet into AcousticEncoder via MSE on FSD50K.
...
Inputs
------
  outputs/fsd50k_cache/eval_mels.npy        log-mel spectrogram cache
  outputs/fsd50k_cache/eval_embeddings.npy  YAMNet embedding cache
  outputs/pca/pca_components.npy            PCA projection matrix
  outputs/pca/pca_mean.npy                  PCA mean vector
...
def main():
    ...
    for path, name in required:
        if not path.exists():
            ...
            print("Run preprocessing/extract_embeddings.py then preprocessing/compute_mels.py first.")
            sys.exit(1)

    print("Loading data …")
    mels       = np.load(FSDCACHE_DIR / "eval_mels.npy")         # (N, 1, 64, 61)
    embeddings = np.load(FSDCACHE_DIR / "eval_embeddings.npy")   # (N, 1024)
    pca_comp   = np.load(PCA_DIR      / "pca_components.npy")    # (32, 1024)
    pca_mean   = np.load(PCA_DIR      / "pca_mean.npy")          # (1024,)
    print(f"  {mels.shape[0]:,} frames loaded")

    # Project teacher embeddings to 32D — these are the distillation targets
    targets = (embeddings - pca_mean) @ pca_comp.T   # (N, 32)
    ...
    train_losses  = []
    val_losses    = []
    ckpt_path     = STUDENT_DIR / "acoustic_encoder.pt"
    ...
    ax.legend()
    ax.grid(alpha=0.3)
    curve_path = STUDENT_DIR / "training_curve.png"
    plt.tight_layout()
